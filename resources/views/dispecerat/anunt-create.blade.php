                        ...
                        @if(count($categorii) === 1)
                            ...
                            <div class="p-2 px-3 d-flex align-items-center gap-2"
                                ...
                                <span style="color:{{ $c[1] }};
                                             font-weight:700;">
                                    {{ $labels[$categorii[0]] ?? ucfirst($categorii[0]) }}
                                </span>
...
pdf:
{ icon:'bi-file-earmark-pdf',color:'#dc2626' },
...
document.getElementById('fisiere-input').files
=dt.files;
...
function afiseazaFisiere()
{

const preview=
document.getElementById('fisiere-preview');
...
iconeTip[ext]
||
{icon:'bi-file-earmark',color:'#64748b'};



const marime=file.size<1048576?(file.size/1024).toFixed(1)+' KB':(file.size/1048576).toFixed(1)+' MB';
...
adaugaFisiere(event.dataTransfer.files);
