...
<head>
    ...
    <title>@yield('title', 'POIM') | Aquaserv Tulcea</title>
    <meta name="description" content="@yield('meta_description', 'Proiectul Regional de dezvoltare a infrastructurii de apă și apă uzată în județul Tulcea — cofinanțat din Fondul de Coeziune, Programul Operațional Infrastructură Mare 2014-2020 (POIM).')">
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Sigle — dimensionate ca pe site-ul sursă (poim.aquaservtulcea.ro): logo-uri mari, distribuite uniform */
        .logos-card { background:#fff; border-radius:16px; box-shadow:0 8px 28px rgba(0,0,0,0.14); padding:2rem; margin:0 auto; }
        .logos-track { display:flex; align-items:center; justify-content:space-evenly; gap:2rem; flex-wrap:wrap; }
        .logo-slot { height:105px; display:flex; align-items:center; justify-content:center; }
        .logo-slot img { height:105px; width:auto; max-width:100%; object-fit:contain; }
        .logo-placeholder { height:105px; width:220px; border:1.5px dashed var(--aqua-border); border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:0.7rem; font-weight:700; color:var(--aqua-gray); text-align:center; padding:0.25rem; text-transform:uppercase; letter-spacing:0.03em; }
        @media (max-width:767.98px) {
            .logos-card { padding:1rem 0.85rem; }
            .logos-track { gap:0.75rem; flex-wrap:nowrap; justify-content:space-between; }
            .logo-slot, .logo-slot img { height:56px; max-height:56px; }
            .logo-placeholder { height:56px; width:95px; font-size:0.6rem; }
        }
...
<body>
    ...
    <div class="hero-photo">
        <div class="container">
            {{-- SIGLE — UE, Guvernul României, POIM 2014-2020 --}}
            <div class="logos-card">
                <div class="logos-track">
                    @foreach ([
                        ...
                        ['file' => 'logo-poim.png',        'alt' => 'Programul Operațional Infrastructură Mare 2014-2020', 'label' => 'Sigla POIM'],
                    ] as $logo)
                        <div class="logo-slot">
                            @if(file_exists(public_path('images/poim/'.$logo['file'])))
                                <img src="{{ asset('images/poim/'.$logo['file']) }}" alt="{{ $logo['alt'] }}">
                            @else
    ...
    <nav class="navbar navbar-expand-lg">
        <div class="container">
            <a class="navbar-brand d-flex align-items-center" href="{{ route('poim.prezentare') }}">
                <img src="{{ asset('images/poim/logo-aquaserv.png') }}" alt="Aquaserv Tulcea" style="height:60px;width:auto;">
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#poimNav">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="poimNav">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{ request()->routeIs('poim.prezentare') ? 'active' : '' }}" href="#" role="button" data-bs-toggle="dropdown">Prezentare</a>
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{ route('poim.despre-poim') }}"><i class="bi bi-info-circle text-aqua me-2"></i>Despre POIM</a></li>
                            <li><a class="dropdown-item" href="{{ route('poim.despre-proiect') }}"><i class="bi bi-file-earmark-text text-aqua me-2"></i>Despre proiect</a></li>
                        </ul>
                    </li>
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{ request()->routeIs('poim.contracte') || request()->routeIs('poim.contract-detaliu') ? 'active' : '' }}" href="{{ route('poim.contracte') }}" role="button" data-bs-toggle="dropdown">Contracte</a>
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{ route('poim.contracte') }}"><i class="bi bi-list-ul text-aqua me-2"></i>Toate contractele</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li class="dropdown-submenu">
                                <a class="dropdown-item" href="#">Lucrări</a>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cl-1') }}">CL-1</a></li>
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cl-4') }}">CL-4</a></li>
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cl-8') }}">CL-8</a></li>
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cl-10') }}">CL-10</a></li>
                                </ul>
                            </li>
                            <li class="dropdown-submenu">
                                <a class="dropdown-item" href="#">Servicii</a>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cs-1') }}">CS-1</a></li>
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cs-2') }}">CS-2</a></li>
                                </ul>
                            </li>
                            <li class="dropdown-submenu">
                                <a class="dropdown-item" href="#">Furnizare</a>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{ route('poim.contract-detaliu', 'cf-2') }}">CF-2</a></li>
                                </ul>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item"><a class="nav-link {{ request()->routeIs('poim.comunicare') ? 'active' : '' }}" href="{{ route('poim.comunicare') }}">Comunicare</a></li>
                    <li class="nav-item"><a class="nav-link {{ request()->routeIs('poim.galerie') ? 'active' : '' }}" href="{{ route('poim.galerie') }}">Galerie</a></li>
                    <li class="nav-item"><a class="nav-link {{ request()->routeIs('poim.contact') ? 'active' : '' }}" href="{{ route('poim.contact') }}">Contact</a></li>
                </ul>
            </div>
        </div>
    </nav>
